...
## 🛣️ Two Paths: Manual vs GitHub Actions Workflows

Each demo can be tested in **two ways**:

| Path | Method | When to Use | Time |
|------|--------|------------|------|
| **Manual** | AWS CLI (`aws s3 cp`, `aws ssm put-parameter`, etc.) | Learning, debugging, quick testing | ~5-10 min |
| **Workflows** ⭐ | GitHub Actions (auto-generated by Terraform) | Production-like testing, CI/CD validation, **recommended** | ~3-5 min per workflow |

**Recommended:** Use **GitHub Actions workflows** — they demonstrate the stack working end-to-end with OIDC auth, exactly as it will in production.
...
Instead of manual AWS CLI commands, use the auto-generated workflow:

```bash
# 1. Commit the generated workflow
git add .github/workflows/deploy.yml
git commit -m "ci: add auto-generated deployment workflow"
git push origin main

# 2. Trigger the workflow (automatic on push or manual)
gh workflow run deploy.yml

# 3. Monitor the workflow
gh run list --workflow deploy.yml
gh run view [RUN_ID]  # See logs and outputs
```

**Result:** Same deployment, but with OIDC authentication and full CI/CD validation — exactly as it would run in production.
...
Use the auto-generated workflows for the full production flow:

```bash
# 1. Deploy with the deploy workflow
gh workflow run deploy.yml
gh run watch  # Wait for completion

# 2. Rollback with one command
...
**Result:** Full blue-green cycle with OIDC auth, same as production deployment.
...
Use all three workflows for the complete production flow:

```bash
# 1. Deploy with the deploy workflow (auto-archives)
gh workflow run deploy.yml
gh run watch
...
**Result:** Full versioning cycle with automatic archiving, OIDC auth, and disaster recovery — production-ready.
