module "gha_gen_workflows" {
  ...
  s3_rollback_bucket_name     = aws_s3_bucket.this[values(local.rollback_bucket)[0].name].id
  s3_versions_bucket_name     = aws_s3_bucket.this[values(local.versioning_buckets)[0].name].id
  ssm_parameter_arn           = aws_ssm_parameter.rollback[0].arn
  ...
  ssm_parameter_name          = aws_ssm_parameter.rollback[0].name
